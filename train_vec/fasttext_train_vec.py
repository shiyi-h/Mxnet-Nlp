...
file_name = "sum_std_info_One.txt"

save_path = "/home/hh/nlp/data/res"
save_name = "007.std.vec"
...
class Train_Vec():
    def __init__(self,dataset,batch_size,loss,init_meth,optimizer,embed_size,num_epochs,lr,n_grams_num):
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch in tqdm(self.data_iter,desc="Epoch_{}".format(epoch+1)):
                centers, context_negatives, masks, labels,batch_size = self._get_batch(batch, ctx)
                with autograd.record():
                    preds = [self.skip_gram(center, context_negative, self.net[0], self.net[1])
                             for center,context_negative in zip(centers,context_negatives)]
                    ls = [(self.loss(pred.reshape(label.shape), label, mask) * mask.shape[1] / mask.sum(axis=1))
                          for pred,label,mask in zip(preds,labels,masks)]
                for l in ls: l.backward()
                trainer.step(batch_size)
                l_sum += sum([l.sum().asscalar() for l in ls])
                n += sum([l.size for l in ls])
            print("epoch %d, loss %.2f, time %.2fs" % (epoch + 1, l_sum / n, time.time() - start))


    def save(self,save_path,save_name):
        w = self.net[0].weight.data(self.ctx[0])
